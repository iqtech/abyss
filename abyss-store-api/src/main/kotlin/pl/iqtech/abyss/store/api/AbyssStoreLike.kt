...
data class StoredEdge(val fromId: NodeId, val toId: NodeId, val edge: SchemaEdgeLike<*>, val remaining: Duration?)

// Stores are keyed by the self-describing NodeId (its bytes are the PK) and hold polymorphic
// NodeLike<*>/SchemaEdgeLike<*> values — one shared store backs every schema in a container, so there
...
interface AbyssStoreLike {
    ...
    suspend fun loadEdge(fromId: NodeId, toId: NodeId, type: String): Either<AbyssError, Pair<SchemaEdgeLike<*>?, Duration?>>
...
interface AbyssStoreTransactionLike {
    ...
    fun saveEdge(fromId: NodeId, toId: NodeId, edge: SchemaEdgeLike<*>)
...
interface AbyssEphemeralStoreTransactionLike {
    ...
    fun saveEdge(fromId: NodeId, toId: NodeId, edge: SchemaEdgeLike<*>, ttl: Duration)
    fun deleteNode(id: NodeId)
    fun deleteEdge(fromId: NodeId, toId: NodeId, type: String)
}
